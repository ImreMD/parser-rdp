...
fn node_builder(material: HashMap<Material, String>, instruction: Build_Instruction)-> (Node, Precedence) {
    ...
    match instruction {
        Build_Instruction::build_left =>  m = Node { operation: op.to_string(), left: Box::new(Some(Leaf::Variable("x".to_string()))), right: Box::new(None) },
        Build_Instruction::build_right =>  m = Node { operation: op.to_string(), left: Box::new(None), right: Box::new(Some(Leaf::Numeric(3))) }}
